...
def load_pretrained_nerf_model(dev, cfg):
    if cfg['pretrained_checkpoint_path'].endswith('.pt'):
        container = torch.jit.load(cfg['pretrained_checkpoint_path'])
        pretrained_nerf = MegaNeRF(
            [getattr(container, 'sub_module_{}'.format(i)) for i in range(len(container.centroids))],
            container.centroids, 1.15, False).to(dev)
        return run_nerf_helpers.ChainImageEmbeddingAndModel(pretrained_nerf, cfg['image_index'])
...
def create_nerf(cfg):
    ...
    model = run_nerf_helpers.NeRF(D=cfg['num_hidden_layers'], W=cfg['hidden_layer_size'],
                                  ...
                                  use_initialization_fix=cfg['use_initialization_fix'])
...
def query_densities(points, pretrained_nerf, cfg, dev):
    ...
    with torch.no_grad():
        ...
        while start < num_points_and_dirs:
            ...
            densities[start:end] = F.relu(pretrained_nerf(points_and_dirs[start:end].to(dev))[:,
                                          -1]).cpu()  # Only select the densities (A) from NeRF's RGBA output
            start = end
    return densities
...
class IterativeMean:
    ...
    def add_values(self, new_values):
        if self.value:
            self.value = (self.num_old_values * self.value + new_values.size(0) * new_values.mean()) / (
