...
from utils import ConfigManager, PerfMonitor, parse_args_and_init_logger, Logger, create_nerf, get_random_directions, \
    load_yaml_as_dict, LPIPS
...
def batchify_rays(rays_flat, chunk=1024 * 32, random_directions=None, background_color=None, **kwargs):
    ...
    for i in range(0, rays_flat.shape[0], chunk):
        ret = render_rays(rays_flat[i:i + chunk], random_directions=random_directions,
...
def render(intrinsics, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
    ...
    if random_direction_probability > 0.:
        ...
        random_viewdirs = torch.tensor(get_random_directions(num_random_viewdirs), dtype=torch.float,
    ...
    # Render and reshape
    all_ret = batchify_rays(rays, chunk, random_directions=random_directions, background_color=background_color,
                            **kwargs)
    if random_directions is not None:
...
def create_fast_kilonerf_renderer(dummy_c2w, intrinsics, render_kwargs):
    ...
    return FastKiloNeRFRenderer(dummy_c2w, intrinsics, render_kwargs['background_color'],
                                render_kwargs['occupancy_grid'],
                                render_kwargs['multi_network'], render_kwargs['domain_mins'],
                                render_kwargs['domain_maxs'],
                                render_kwargs['white_bkgd'], render_kwargs['N_samples'], render_kwargs['near'],
                                render_kwargs['far'],
                                has_flag(cfg, 'performance_monitoring'), cfg['occupancy']['resolution'],
                                cfg['render']['max_samples_per_ray'], cfg['render']['transmittance_threshold'])
...
def render_to_screen(intrinsics, render_kwargs):
    ...
    orbit_camera = OrbitCamera(center, orbit_camera_cfg['radius'], orbit_camera_cfg['inclination'],
                               orbit_camera_cfg['azimuth'], device)
...
def render_path(render_poses, intrinsics, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    c2ws = [x[:3, :4].cuda() for x in render_poses]
    ...
    if use_fast_sampling:
        fast_kilonerf_renderer = create_fast_kilonerf_renderer(c2ws[0], intrinsics[0], render_kwargs)
    ...
    for i, c2w in enumerate(tqdm(c2ws)):
        ...
        if use_fast_sampling:
            if i > 0 and (intrinsics[i].H != intrinsics[i - 1].H) or (intrinsics[i].W != intrinsics[i - 1].W):
                print('Setting new RGB map')
                rgb_map = torch.empty([intrinsics[i].H, intrinsics[i].W, 3], dtype=torch.float,
                                      device=fast_kilonerf_renderer.rgb_map.device)
                fast_kilonerf_renderer.rgb_map = rgb_map
                fast_kilonerf_renderer.rgb_map_pointer = rgb_map.data_ptr()

            fast_kilonerf_renderer.intrinsics = intrinsics[i]

        if use_fast_sampling:
            fast_kilonerf_renderer.set_camera_pose(c2w)
            rgb, elapsed_time = fast_kilonerf_renderer.render()
        else:
            rgb, disp, acc, elapsed_time = render(intrinsics[i], chunk=chunk, c2w=c2w, **render_kwargs)[:4]
        ...
        if calculate_quality_metrics:
            ...
            if num_nans > 0:
                print(
                    'WARNING: Rendered image contains {} nan values. Converting nans to (1., 0., 0.)'.format(num_nans))
            ...
            log_str += 'MSE: {:.6f}, PSNR: {:.3f}, SSIM: {:.3f}, LPIPS: {:.3f}, '.format(mse.item(), psnr.item(), ssim,
                                                                                         lpips.item())
    ...
    Logger.write(
        'average over image batch. MSE: {:.6f}, PSNR: {:.3f}, SSIM: {:.3f}, LPIPS: {:.3f}, elapsed time: {:7.2f} ms'.format(
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, background_color=None, pytest=False,
    ...
    if no_color_sigmoid:
        rgb = torch.min(torch.max(raw[..., :3], torch.zeros_like(raw[..., :3])),
                        torch.ones_like(raw[..., :3]))  # [N_rays, N_samples, 3]
    else:
...
def render_rays(ray_batch,
    ...
    def query_network():
        if network_query_fn is not None:
            ...
            return query_multi_network(multi_network, domain_mins, domain_maxs, pts, viewdirs,
                                       position_fourier_embedding, direction_fourier_embedding, occupancy_grid,
                                       debug_network_color_map, use_network_jittering, random_directions, cfg)
    ...
    if has_flag(cfg, 'visualize_global_domain'):
        ...
        inside_global_domain = torch.logical_and((pts > global_domain_min + epsilon).all(dim=2),
                                                 (pts < global_domain_max - epsilon).all(dim=2))
    ...
    rgb_map, disp_map, acc_map, weights, depth_map, alpha, transmittance = raw2outputs(raw, z_vals, rays_d,
                                                                                       raw_noise_std, white_bkgd,
                                                                                       background_color,
                                                                                       pytest=pytest,
                                                                                       no_color_sigmoid=no_color_sigmoid)
    ...
    if N_importance > 0:
        ...
        if 'custom_color_min' in cfg:
            ...
            inside_box = torch.logical_and(
                (torch.tensor(cfg['custom_color_min'], dtype=torch.float, device=device) <= pts_flat).all(dim=1),
...
def pretrain_constant(multi_network, position_fourier_embedding, direction_fourier_embedding, cfg):
    ...
    batch_size_per_network = torch.tensor([batch_size] + [0 for _ in range(multi_network.num_networks - 1)],
                                          dtype=torch.long, device=device).cpu()

    # Positional encoding
    fourier_embedding_implementation = 'custom_kernel_v2'  # pytorch
    if position_fourier_embedding is not None:
        embedded_points = position_fourier_embedding(random_directions.unsqueeze(0),
    ...
    if direction_fourier_embedding is not None:
        embedded_dirs = direction_fourier_embedding(random_directions.unsqueeze(0),
                                                    implementation=fourier_embedding_implementation).squeeze(0)
    else:
...
def train(cfg, log_path, render_cfg_path):
    ...
    Logger.write('global_domain_min: {}, global_domain_max: {}, near: {}, far: {}, background_color: {}'.format(
        global_domain_min, global_domain_max, near, far, background_color))
    ...
    if cfg['model_type'] == 'single_network':
        ...
        position_num_input_channels, position_fourier_embedding = create_multi_network_fourier_embedding(1, cfg[
            'num_frequencies'])
        direction_num_input_channels, direction_fourier_embedding = create_multi_network_fourier_embedding(1, cfg[
            'num_frequencies_direction'])
        ...
        if not load_from_distilled:
            ...
            model = multi_network = create_multi_network(num_networks, position_num_input_channels,
                                                         direction_num_input_channels, 4,
        ...
        # From distillation
            ...
            model = multi_network = build_multi_network_from_single_networks(single_networks,
                                                                             linear_implementation=linear_implementation,
                                                                             view_dependent_dropout_probability=cfg[
                                                                                 'view_dependent_dropout_probability']).to(
                device)
            multi_network.activation = nn.ReLU(
                inplace=True)  # TODO: make sure that other activation functions are also inplace
            domain_mins = torch.tensor(domain_mins, device=device)
            domain_maxs = torch.tensor(domain_maxs, device=device)
        ...
        if 'occupancy_cfg_path' in cfg:
            ...
            if 'render' in cfg:
                ...
                for dim in range(3):
                    occupancy_voxel_centers.append(
                        torch.linspace(global_domain_min[dim] + occupancy_voxel_half_size[dim],
                ...
                if 'tight_domain_min' in cfg['render'] and 'tight_domain_max' in cfg['render']:
                    print('[WARNING] Cropping domain to:', cfg['render']['tight_domain_min'],
                          cfg['render']['tight_domain_max'])
                    ...
                    inside_tight_domain = torch.logical_and(torch.all(tight_domain_min < occupancy_domain_mins, dim=1),
                                                            torch.all(occupancy_domain_maxs < tight_domain_max, dim=1))
                ...
                if has_flag(cfg['render'], 'fast_sampling'):
                    ...
                    assigned_network_grid = ((occupancy_voxel_centers - global_domain_min) / network_voxel_size).to(
                        strides)
    ...
    if has_flag(cfg, 'render_to_screen'):
        render_to_screen(val_intrinsics[0], render_kwargs_test)
        return
    ...
    if cfg['render_only']:
        ...
        with torch.no_grad():
            ...
            print('test poses shape', len(val_poses), len(val_intrinsics), val_intrinsics[0].H, val_intrinsics[0].W,
                  val_intrinsics[0].fx, val_intrinsics[0].fy, val_intrinsics[0].cx, val_intrinsics[0].cy)

            rgbs, _ = render_path(val_poses, val_intrinsics, cfg['chunk_size'], render_kwargs_test, gt_imgs=val_images,
    ...
    if has_flag(cfg, 'rng_seed_fix') and load_from_checkpoint and 'torch_rng_state' in cp:
        Logger.write('Loading rng state. torch: {}, torch cuda: {}, numpy: {}'.format(cp['torch_rng_state'].sum(),
                                                                                      cp['torch_cuda_rng_state'].sum(),
                                                                                      cp['numpy_rng_state'][1].sum()))
    ...
    for i in trange(start, N_iters):
        ...
        if cur_loaded is None or i_batch >= cur_loaded.shape[0] - N_rand:
            next_chunk = next(chunk_index)
            print(chunks[next_chunk])
            cur_loaded = torch.FloatTensor(np.load(chunks[next_chunk]))
            print('loaded')
            loaded_indices = np.arange(len(cur_loaded))
            np.random.shuffle(loaded_indices)
            i_batch = 0

        batch_indices = loaded_indices[i_batch:i_batch + N_rand]
        target_s = cur_loaded[batch_indices, :3].to(device, non_blocking=True)
        batch_rays = cur_loaded[batch_indices, 3:6].to(device, non_blocking=True), cur_loaded[batch_indices, 6:9].to(
        ...
        #             if i == start:
        #                 Logger.write(
        #                     f"[Config] Center cropping of size {2 * dH} x {2 * dW} is enabled until iter {cfg['precrop_iterations']}")
        #         else:
        #             coords = torch.stack(torch.meshgrid(torch.linspace(0, intrinsics.H - 1, intrinsics.H),
        #                                                 torch.linspace(0, intrinsics.W - 1, intrinsics.W)),
        #                                  -1)  # (H, W, 2)
        #
        ...
        if 'mean_regularization' in cfg:
            random_directions = torch.tensor(get_random_directions(cfg['mean_regularization']['num_random_directions']),
                                             dtype=torch.float, device=device)
            random_directions = direction_fourier_embedding(random_directions.unsqueeze(0),
                                                            implementation='custom_kernel_v2').squeeze()
        ...
        if do_constant_pretraining:  # assuming local coordinates
            ...
            all_ret = render(val_intrinsics[0], chunk=cfg['chunk_size'], rays=batch_rays,
        ...
        if i % cfg['render_video_interval'] == 0 and i > 0:
            ...
            model.eval()
            with torch.no_grad():
                rgbs, disps = render_path(val_poses, val_intrinsics, cfg['chunk_size'], render_kwargs_test)
            print('Done, saving', len(rgbs), len(disps))
        ...
        if i % cfg['render_testset_interval'] == 0 and i > 0:
            ...
            model.eval()
            with torch.no_grad():
                render_path(val_poses, val_intrinsics, cfg['chunk_size'],
                            render_kwargs_test, gt_imgs=val_images, savedir=testsavedir)
        ...
        if i % cfg['checkpoint_interval'] == 0:
            ...
            if has_flag(cfg, 'rng_seed_fix'):
                Logger.write('Saving rng state. torch: {}, torch cuda: {}, numpy: {}'.format(torch_rng_state.sum(),
                                                                                             torch_cuda_rng_state.sum(),
                                                                                             numpy_rng_state[1].sum()))
            cp = {
